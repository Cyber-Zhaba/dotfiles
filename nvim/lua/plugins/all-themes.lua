-- Omarchy hot-reloads nvim when the system theme changes, so every colorscheme
-- it might pick has to already be on disk. Off Omarchy nothing ever selects
-- these, and installing them would clone ~20 repositories for nothing.
if not (vim.uv or vim.loop).fs_stat(vim.fn.expand("~/.local/state/omarchy")) then
	return {}
end
